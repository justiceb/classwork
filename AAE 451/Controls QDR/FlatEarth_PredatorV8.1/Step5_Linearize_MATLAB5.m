% Step5_Linearize_MATLAB5.m   Version 8.0 1/23/03. 
%
...
disp(' ');disp('>>>>>>>Start here, Step5_Linearize_MATLAB5<<<<<<<');disp(' ');
disp('Compute a quick trim')

Vt=constant(58)  % trim speed (ft/sec) 
Vtknots=Vt/1.688 % trim speed (knots)
...
[x1,u1,CL,CD,CM,alphadeg]=QuickTrim2(Vt,Hp,constant,Vwx,Vwy,Vwz)

% Check to insure that the first 9 derivatives of the state vector (xdot) are small 
% for this trim condition.
flag=1; t=0;
[xdot,x0,str,ts] = aircraft7(t,x1,u1,flag,constant,x1);

% Get outputs at the trim condition
flag=3; t=0;
[y1,x0,str,ts] = aircraft7(t,x1,u1,flag,constant,x1);
...
echo off
[a,b,c,d]=linmod('FlatEarth_ports_MATLAB5',x1,u1) %Find linear model of nonlinear pendulum
